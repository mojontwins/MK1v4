' mkts_om v0.11.20250501 - Converts graphics
' Copyleft 2017-2025 The Mojon Twins
...
Sub usage
	...
	Puts "                 superbuffer, palsasassembly"
...
Sub doPals (img As Any Ptr, prefix As String, outputFn As String)
	...
	Select Case platform
		Case PLATFORM_CPC:
			...
			Print #f, "// Generated by mkts_om v0.11.20250501"
			Print #f, "// Copyleft 2017-2025 by The Mojon Twins"
			Print #f, ""
			If palsAsAssembly Then
				Print #f, "#asm"
				Print #f, "	." & prefix
				Print #f, "		ld  b, 0x7f"
				For i = 0 To mc-1
					...
					Print #f, "		ld  a, " & i
					Print #f, "		out (c), a"
					Print #f, "		ld  a, 0x" & Hex(((c And &H1F) Or 64), 2) & "	// 0b0100000 OR " & Hex(c) & " (" & Hex (globalPalette(i)) & ")"
					Print #f, "		out (c), a"
				Next i
				Print #f, "		ret"
				Print #f, "#endasm" 
...
Sub doChars (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, max As Integer)
	...
	Puts "mkts_om v0.11.20250501 ~ Chars mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes)."
End Sub
...
Sub doStrait2x2(img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, max As Integer)
	...
	Puts "mkts_om v0.11.20250501 ~ Strait2x2 mode, " & ct*4 & " patterns extracted (" & (ct*4 * patternSize) & " bytes)."
End Sub
...
Sub cpcDoSuperBuffer (img As Any Ptr)
	...
	Puts "mkts_om v0.11.20250501 ~ Superbuffer mode (12288 bytes)."
End Sub
...
Sub cpcDoScr (img As Any Ptr)
	...
	Puts "mkts_om v0.11.20250501 ~ Superbuffer mode (16384 bytes)."
...
Sub doTmaps (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, wMeta As Integer, hMeta As Integer, max As Integer)
	...
	Puts "mkts_om v0.11.20250501 ~ Tmaps mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes) from " & ctTmaps & " metatiles (" & (ctTmaps * i * wMeta * hMeta) & " bytes)."
End Sub
...
zxDoSpritesBreak:
	...
	cutSpriteCount = ct
	Puts "mkts_om v0.11.20250501 ~ Sprites mode, " & ct & " " & wMetaPixels & "x" & hMetaPixels & " sprite cells with masks extracted (" & (mainIndex) & " bytes)."
End Sub
...
cpcDoSpritesBreak:
	cutSpriteCount = ct
	Puts "mkts_om v0.11.20250501 ~ Sprites mode, " & ct & " " & wMetaPixels & "x" & hMetaPixels & " sprite cells extracted (" & (mainIndex - curIndex) & " bytes)."
End Sub
...
Sub generateMixedMappings (mappingsFn As String)
	...
	For i = 0 To spriteMetaIndex - 1
		...
		Else 
			functionSuffix = "" & (spriteMetaData (i).w * 2) & "Bx" & (spriteMetaData (i).h * 8)
		End If
...
Sub zxDoBg (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer)
	...
	Puts "mkts_om v0.11.20250501 ~ BG mode, " & ct & " patterns (" & (8*ct) & " bytes). NT is " & auxIndex & " bytes."
End Sub
...
Sub cpcDoNametable (img As Any Ptr)
	...
	Puts "mkts_om v0.11.20250501 ~ Nametable mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes)."
End Sub
...
Sub doNametableRLE (img As Any Ptr)
	...
	Puts "mkts_om v0.11.20250501 ~ Rle encoder, compressed to " & rleIdx & " bytes."
End Sub
...
If sclpGetValue ("platform") = "cpc" Then
	...
	If sclpGetValue ("mode") <> "scripted" And sclpGetValue ("mode") <> "pal" And sclpGetValue ("mode") <> "outputpal" And sclpGetValue ("mode") <> "pals" And sclpGetValue ("mode") <> "palsasassembly" Then
