' mkts_om v0.10.20250415 - Converts graphics
...
Sub doPals (img As Any Ptr, prefix As String, outputFn As String)
	...
	Select Case platform
		Case PLATFORM_CPC:
			...
			Print #f, "// Generated by mkts_om v0.10.20250415"
			...
			If palsAsAssembly Then
				...
				For i = 0 To 3
					...
					Print #f, "		out (c), " & i
					Print #f, "		out (c), 0x" & Hex(((c And &H1F) Or 64), 2) & "	// 0b0100000 OR " & Hex(c) & " (" & Hex (globalPalette(i)) & ")"
				Next i
				Print #f, "		ret"
				Print #f, "#endasm" 
			Else
...
Sub doChars (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, max As Integer)
	...
	Puts "mkts_om v0.10.20250415 ~ Chars mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes)."
End Sub
...
Sub doStrait2x2(img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, max As Integer)
	...
	Puts "mkts_om v0.10.20250415 ~ Strait2x2 mode, " & ct*4 & " patterns extracted (" & (ct*4 * patternSize) & " bytes)."
End Sub
...
Sub cpcDoSuperBuffer (img As Any Ptr)
	...
	Puts "mkts_om v0.10.20250415 ~ Superbuffer mode (12288 bytes)."
End Sub
...
Sub cpcDoScr (img As Any Ptr)
	...
	Puts "mkts_om v0.10.20250415 ~ Superbuffer mode (16384 bytes)."
...
Sub doTmaps (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, wMeta As Integer, hMeta As Integer, max As Integer)
	...
	Puts "mkts_om v0.10.20250415 ~ Tmaps mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes) from " & ctTmaps & " metatiles (" & (ctTmaps * i * wMeta * hMeta) & " bytes)."
End Sub
...
zxDoSpritesBreak:
	...
	cutSpriteCount = ct
	Puts "mkts_om v0.10.20250415 ~ Sprites mode, " & ct & " " & wMetaPixels & "x" & hMetaPixels & " sprite cells with masks extracted (" & (mainIndex) & " bytes)."
End Sub
...
cpcDoSpritesBreak:
	cutSpriteCount = ct
	Puts "mkts_om v0.10.20250415 ~ Sprites mode, " & ct & " " & wMetaPixels & "x" & hMetaPixels & " sprite cells extracted (" & (mainIndex - curIndex) & " bytes)."
End Sub
...
Sub zxDoBg (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer)
	...
	Puts "mkts_om v0.10.20250415 ~ BG mode, " & ct & " patterns (" & (8*ct) & " bytes). NT is " & auxIndex & " bytes."
End Sub
...
Sub cpcDoNametable (img As Any Ptr)
	...
	Puts "mkts_om v0.10.20250415 ~ Nametable mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes)."
End Sub
...
Sub doNametableRLE (img As Any Ptr)
	...
	Puts "mkts_om v0.10.20250415 ~ Rle encoder, compressed to " & rleIdx & " bytes."
End Sub
