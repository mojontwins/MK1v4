	...
	while (1) {
		...
		while (playing) {
			...
			if (gpy == 0 && player.vy < 0 && n_pant >= MAP_W
				...
				// lets you navigate the map. Trigger key is KEY_AUX2 (x) in CPC, 0x02fd (s) in ZX.
				if (
				#ifdef CPC
					cpc_TestKey (KEY_AUX2) 
				#else
					sp_KeyPressed (0x02fd)
				#endif
				) {
					if ((pad_this_frame & sp_UP) == 0 && n_pant >= MAP_W) { n_pant -= MAP_W; }
